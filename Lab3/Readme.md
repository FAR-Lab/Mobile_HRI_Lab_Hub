...
## Lab Safety
- The hoverboard metal chassis is **conductive**. **Do not place any electronics on it, including the ODrive board.** Otherwise, you will see sparks and short the board.
- Dangling wire is always dangerous. They can short your circuit in no time. Cover them with electrical tape.
- Be careful with the hoverboard battery. **Do not leave the lab with the batteries plugged in to ODrive.** The ODrive will keep draining your battery even if the battery is below its nominal voltage, and your battery can be damaged permanently.
...
> For students in Ithaca, follow the video [here](https://drive.google.com/file/d/14AAURnUGDazZhO_vaSGaS3nCYTV7YWQE/view?usp=sharing) to take apart your hoverboard. After fully disassembling your hoverboard, please do the following
...
2. Take a XT60 connector (pay attention to the shape), insert it to ODrive and tighten it. Be careful with the polarity, **red wire connects to positive, black wire connects to negative.**
...
(Only one person needs to do this. ODrive cannot be openned on multiple threads.)
```bash
...
`odrv0.axis0` and `odrv0.axis1` refer to the two axes on ODrive, each axis is used to controll one hub motor/wheel. `dump_errors(odrv0)` is a handy command to show current errors.`odrv0.clear_errors()` is a useful command to reset your odrive errors.      
...
dump_errors(odrv0) # If there is an error under axis0, rertun calibration `odrv0.axis0.requested_state = AXIS_STATE_FULL_CALIBRATION_SEQUENCE`
...
odrv0.reboot()
```
If you get an error regarding "HALL ENCODER STATE", check if your filtering PCB is plugged in correctly. If the error persists, let the TA know. 
...
- Your robot will interact with people. 
